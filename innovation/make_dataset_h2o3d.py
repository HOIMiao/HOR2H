...
parser.add_argument("--YCBModelsDir", default='D:\Hand_Object_pose_shape\THOR-Net-ours\datasets\ycb_models',
                    help="Path to YCB object meshes folder")
parser.add_argument("--dataset_path", default='D:\Hand_Object_pose_shape\THOR-Net-ours\datasets\ho3d_v2\data',
...
def load_mesh_from_manolayer(fullpose, beta, trans, mano_layer):
    ...
    # Pass to Mano layer
    hand_verts, hand_joints, _ = mano_layer(fullpose_tensor, shape_tensor, trans_tensor)
...
def load_annotations(data, right_mano_layer, left_mano_layer, subset='train'):
    cam_intr = data['camMat']

    if subset == 'train':
        lefthand3d = data['leftHandJoints3D'][reorder_idx]
        righthand3d = data['rightHandJoints3D'][reorder_idx]
    else:
        lefthand3d = data['leftHandJoints3D'].reshape((1, -1))
        righthand3d = data['rightHandJoints3D'].reshape((1, -1))
    ...
    hand_object3d = np.concatenate([lefthand3d, righthand3d, obj_corners]) * 1000
    ...
    mesh2d = np.array([])
    if subset == 'train':
        _, lefthand_mesh3d, _ = load_mesh_from_manolayer(data['leftHandPose'], data['handBeta'], data['leftHandTrans'], left_mano_layer)
        _, righthand_mesh3d, _ = load_mesh_from_manolayer(data['rightHandPose'], data['handBeta'], data['rightHandTrans'], right_mano_layer)

        # Project from 3D world to Camera coordinates using the camera matrix
        lefthand_mesh3d = lefthand_mesh3d.dot(coordChangeMat.T)
        righthand_mesh3d = righthand_mesh3d.dot(coordChangeMat.T)
        lefthand_mesh_proj = cam_intr.dot(lefthand_mesh3d.transpose()).transpose()
        righthand_mesh_proj = cam_intr.dot(righthand_mesh3d.transpose()).transpose()
        lefthand_mesh2d = (lefthand_mesh_proj / lefthand_mesh_proj[:, 2:])[:, :2]
        righthand_mesh2d = (righthand_mesh_proj / righthand_mesh_proj[:, 2:])[:, :2]
        ...
        mesh3d = np.concatenate((lefthand_mesh3d, righthand_mesh3d, obj_mesh3d), axis=0)
        mesh2d = np.concatenate((lefthand_mesh2d, righthand_mesh2d, obj_mesh2d), axis=0)
...
if __name__ == '__main__':
    ...
    right_mano_layer = ManoLayer(mano_root=mano_root, use_pca=False, ncomps=6, flat_hand_mean=True)
    left_mano_layer = ManoLayer(mano_root=mano_root, side='left', use_pca=False, ncomps=6, flat_hand_mean=True)
    names = ['images', 'depths', 'points2d', 'points3d', 'mesh3d', 'mesh2d']
    file_dict_train = defaultdict(list)
    file_dict_val = defaultdict(list)
    name_object_dict = {}

    val_list = ['MBC4', 'SHB1', 'SHP10', 'SMB11', 'SSBC4', 'SSPD4']
    ...
    for subject in tqdm(sorted(os.listdir(os.path.join(train)))):
        ...
        for rgb_file in sorted(os.listdir(rgb)):
            ...
            if data['leftHandJoints3D'] is None and data['rightHandJoints3D'] is None:
                # Load previous frame's data if data is missing
                count += 1
                continue
            else:
                hand_object3d, hand_object2d, mesh3d, mesh2d = load_annotations(data, right_mano_layer, left_mano_layer)
    ...
    for subject in tqdm(os.listdir(os.path.join(evaluation))):
        ...
        for rgb_file in os.listdir(rgb):
            ...
            if data['leftHandJoints3D'] is None and data['rightHandJoints3D'] is None:
                ...
                hand_object3d, hand_object2d, mesh3d, mesh2d = load_annotations(data, right_mano_layer, left_mano_layer, subset='test')
