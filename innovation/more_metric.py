...
def calculate_fscore(gt, pr, th=0.01):
    gt = verts2pcd(gt)
    pr = verts2pcd(pr)
    d1 = o3d.geometry.PointCloud.compute_point_cloud_distance(gt, pr) # closest dist for each gt point
    d2 = o3d.geometry.PointCloud.compute_point_cloud_distance(pr, gt) # closest dist for each pred point
