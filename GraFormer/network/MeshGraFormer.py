...
#-------------innovation-----------------------
from innovation.sem_graph_conv import _ResSemGraphConv
...
class MeshGraFormer(nn.Module):
    def __init__(self, initial_adj, coords_dim=(2, 3), hid_dim=128, num_layers=5, n_head=4,  dropout=0.1, num_kps3d=50, num_verts=2556, adj_matrix_root='D:/Hand_Object_pose_shape/THOR-Net-ours/GraFormer/adj_matrix'):
        ...
        hid_dim_list = [hid_dim, hid_dim // 4, hid_dim // 16, coords_dim[1]]  #原
        # hid_dim_list = [coords_dim[0], coords_dim[0], coords_dim[0], coords_dim[1]]   #改  且参数n_head由4改1
        ...
        for i in range(self.num_points_levels):
            ...
            self.m = (self.adj[i] > 0)  #self.m是语义图卷积里面需要的一个参数，因为当传入adj到图卷积层中后，adj因为不知明原因会成为多个tensor对象（实际上应该是一个tensor对象，而我发现在adj传入到图卷积层之前是正常的，因此在此对语义图卷积层中需要的参数self.m做运算然后将其作为参数传入到语义图卷积层中使用，此操作不会影响任何结果）
            gconv_inputs.append(ChebConv(in_c=hid_dim, out_c=hid_dim, K=2))
            
            attn = MultiHeadedAttention(n_head, hid_dim)
            gcn = GraphNet(self.adj[i], self.m, in_features=hid_dim, out_features=hid_dim, n_pts=points_levels[i])
            attention_layer = []
            gconv_layer = []
            # self.m = (self.adj[i] > 0)  #self.m是语义图卷积里面需要的一个参数，因为当传入adj到图卷积层中后，adj因为不知明原因会成为多个tensor对象（实际上应该是一个tensor对象，而我发现在adj传入到图卷积层之前是正常的，因此在此对语义图卷积层中需要的参数self.m做运算然后将其作为参数传入到语义图卷积层中使用，此操作不会影响任何结果）
            ...
            for j in range(num_layers):
                ...
                #---------------------------------------------------
                # 往GraFormer中加入语义卷积模块，包含3个语义卷积层
                _semgconv_layers.append(_ResSemGraphConv(input_dim=hid_dim, output_dim=hid_dim, adj=self.adj[i], m=self.m))
                #---------------------------------------------------
        ...
        #-------------------------------------
        # 语义卷积模块
        self.semgconv_layers = nn.ModuleList(_semgconv_layers)
        #-------------------------------------
    ...
    def forward(self, x):
        ...
        for i in range(self.num_points_levels):
            ...
            for j in range(self.n_layers):
                ...
                #--------加入语义卷积-------------------
                out = self.semgconv_layers[i * self.n_layers + j](out)
                #-------------------------------------
