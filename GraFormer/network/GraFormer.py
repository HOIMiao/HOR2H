...
#-------------innovation-----------------------
from innovation.sem_graph_conv import _ResSemGraphConv, SemGraphConv
...
class GraphNet(nn.Module):
    ...
    def __init__(self, adj, m, in_features=2, out_features=2, n_pts=21):
        ...
        self.gconv1 = SemGraphConv(in_features, in_features * 2, adj, m)
        # self.gconv2 = LAM_Gconv(in_features * 2, out_features, activation=None)
        self.gconv2 = SemGraphConv(in_features * 2, out_features, adj, m)
...
class GraFormer(nn.Module):
    def __init__(self, adj, hid_dim=128, coords_dim=(2, 3), num_layers=4,
        ...
        self.adj = adj
        self.m = (self.adj > 0) #self.m是语义图卷积里面需要的一个参数，因为当传入adj到图卷积层中后，adj因为不知明原因会成为多个tensor对象（实际上应该是一个tensor对象，而我发现在adj传入到图卷积层之前是正常的，因此在此对语义图卷积层中需要的参数self.m做运算然后将其作为参数传入到语义图卷积层中使用，此操作不会影响任何结果）
        ...
        gcn = GraphNet(self.adj, self.m, in_features=dim_model, out_features=dim_model, n_pts=n_pts)
        ...
        for i in range(num_layers):
            ...
            _semgconv_layers.append(_ResSemGraphConv(input_dim=hid_dim, output_dim=hid_dim, adj=self.adj, m=self.m))
            #---------------------------------------------------
    ...
    def forward(self, x):
        ...
        for i in range(self.n_layers):
            ...
            #------------------------------------
            #语义卷积层运算
            out = self.semgconv_layers[i](out)
            #------------------------------------
