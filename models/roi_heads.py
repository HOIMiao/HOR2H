    ...
    def forward(self,
        ...
        if self.keypoint_roi_pool is not None and self.keypoint_head is not None \
            ...
            if self.num_verts > 0 and ((self.num_classes > 2 and filtered_keypoint_proposals is not None) or batch > 0):
                ...
                #-------------------------------------------------------------------
                ##self enhancement transformer ，一个自注意力模块，用来特增增强
                # graformer_inputs = self.SET(graformer_inputs)   #加入SET
                #-------------------------------------------------------------------
                ...
                pose_graformer_inputs = mesh_graformer_inputs

                # Estimate 3D pose
                keypoint3d = self.keypoint_graformer(graformer_inputs)  #得到3D pose的输出（heatmaps作为输入）
                # keypoint3d = self.keypoint_graformer(pose_graformer_inputs)  #得到3D pose的输出(featuremap+heatmaps作为输入)

                # Pass features and pose to Coarse-to-fine GraFormer
                mesh3d = self.mesh_graformer(mesh_graformer_inputs)     #得到3D shape的输出
                # mesh3d = self.mesh_graformer(keypoint3d)     #得到3D shape的输出(将关节点结果作为输入，也就是pose和mesh分支串联)
            ...
            if self.training:
                ...
                # print(keypoint_proposals)#8x1x4
                # print(gt_keypoints)#8x21x3
                # print(pos_matched_idxs)
                # print(keypoint3d.shape)
                # print(keypoints3d_gt.shape)#8x21x3
                # print(mesh3d.shape)
                # print(mesh3d_gt.shape)
                # print(original_imgs.shape)
                # print(palms_gt.shape)
