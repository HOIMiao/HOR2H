...
from innovation.maxMSE_loss import maxmse_loss
...
def keypointrcnn_loss(keypoint_logits, proposals, gt_keypoints, keypoint_matched_idxs, 
                    ...
                    photometric=False, num_classes=4, dataset_name='h2o3d'):
    ...
    keypoint3d_loss = F.mse_loss(keypoint3d_pred, keypoint3d_targets) / 1000  #使用MSE损失函数计算损失
    # keypoint3d_loss = F.smooth_l1_loss(keypoint3d_pred, keypoint3d_targets) / 1000  #使用smooth_l1_loss损失函数(与δ=1的Huber损失函数等价)计算损失
    # keypoint3d_loss = maxmse_loss(keypoint3d_pred, keypoint3d_targets) / 1000   #使用maxMSE损失函数计算损失
    ...
    mesh3d_loss = F.mse_loss(mesh3d_pred, mesh3d_targets) / 1000  #使用MSE损失函数计算损失
    # mesh3d_loss = F.smooth_l1_loss(mesh3d_pred, mesh3d_targets) / 1000  #使用smooth_l1_loss损失函数(与δ=1的Huber损失函数等价)计算损失
    # mesh3d_loss = maxmse_loss(mesh3d_pred, mesh3d_targets) / 1000   #使用maxMSE损失函数计算损失
...
def calculate_photometric_loss(pts3D, rgb, images, N, K, dataset_name='h2o3d', centers=None):
    ...
    photometric_loss = F.mse_loss(rgb, pixels)    #使用MSE损失函数计算损失
    # photometric_loss = F.smooth_l1_loss(rgb, pixels)    #使用smooth_l1_loss损失函数(与δ=1的Huber损失函数等价)计算损失
    # photometric_loss = maxmse_loss(rgb, pixels) #使用maxMSE损失函数计算损失
